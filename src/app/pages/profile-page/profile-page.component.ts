    ...
    private getCardCoverageText(subscription: any): string {
        ...
        switch (coverage) {
            ...
            case 'governorate': {
                ...
                // لو اللوكيشن نفسه محافظة
                if (userLocation.type === 'governorate') {
                    return String(userLocation.name).trim();
                }
                // لو zone، نجيب الـ parent من الـ locations list
                if (userLocation.parent_id) {
                    const parent = this.locations.find(l => String(l.id) === String(userLocation.parent_id));
                    if (parent) return String(parent.name).trim();
                }
                // لو مفيش parent يرجع هو نفسه
